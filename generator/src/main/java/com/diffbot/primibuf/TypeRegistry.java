...
import com.diffbot.primibuf.RequestInfo.TypeInfo;
...
class TypeRegistry {
    ...
    TypeName resolveJavaTypeFromProto(FieldDescriptorProto descriptor) {
        ...
        throw new com.diffbot.primibuf.GeneratorException("Unsupported type: " + descriptor);
    }
    ...
    void registerContainedTypes(com.diffbot.primibuf.RequestInfo info) {
        typeMap.clear();
    ...
    private void registerType(TypeInfo typeInfo) {
        if (typeMap.containsValue(typeInfo.getTypeName()))
            throw new com.diffbot.primibuf.GeneratorException("Duplicate class name: " + typeInfo.getTypeName());
        if (typeMap.put(typeInfo.getTypeId(), typeInfo.getTypeName()) != null)
            throw new com.diffbot.primibuf.GeneratorException("Duplicate type id: " + typeInfo.getTypeId());
