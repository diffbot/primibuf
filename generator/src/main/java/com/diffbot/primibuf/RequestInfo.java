...
import static com.diffbot.primibuf.Preconditions.*;
...
public class RequestInfo {
    ...
    public RequestInfo(List<FileDescriptorProto> descriptors) {
        this.generatorParameters = new HashMap<>();
        this.files = descriptors.stream()
                .map(desc -> new FileInfo(this, desc))
                .collect(Collectors.toList());
        this.typeRegistry.registerContainedTypes(this);
    }
    ...
    public static class FileInfo {
        ...
        FileInfo(RequestInfo parentRequest, FileDescriptorProto descriptor) {
            ...
            // we distinguish this class with a ReadOnly prefix
            outerClassName = ClassName.get(javaPackage, "ReadOnly" + NamingUtil.getJavaOuterClassname(descriptor));
    ...
    public static class MessageInfo extends TypeInfo {
        ...
        MessageInfo(FileInfo parentFile, String parentTypeId, ClassName parentType, boolean isNested, DescriptorProtos.DescriptorProto descriptor) {
            ...
            List<FieldDescriptorProto> sortedFields = descriptor.getFieldList().stream()
                    .sorted(com.diffbot.primibuf.FieldUtil.MemoryLayoutSorter)
                    .collect(Collectors.toList());
            ...
            for (FieldDescriptorProto desc : sortedFields.stream()
                    .sorted(com.diffbot.primibuf.FieldUtil.GroupOneOfAndRequiredBits)
                    .collect(Collectors.toList())) {
    ...
    public static class FieldInfo {
        ...
        FieldInfo(FileInfo parentFile, MessageInfo parentTypeInfo, ClassName parentType, FieldDescriptorProto descriptor, int bitIndex) {
            ...
            isPrimitive = com.diffbot.primibuf.FieldUtil.isPrimitive(descriptor.getType());
            tag = com.diffbot.primibuf.FieldUtil.makeTag(descriptor);
            bytesPerTag = com.diffbot.primibuf.FieldUtil.computeRawVarint32Size(tag);
            packedTag = com.diffbot.primibuf.FieldUtil.makePackedTag(descriptor);
            number = descriptor.getNumber();
            fieldName = NamingUtil.filterKeyword(lowerName);
            final String defValue = com.diffbot.primibuf.FieldUtil.getDefaultValue(descriptor);
            defaultValue = isEnum() ? NamingUtil.filterKeyword(defValue) : defValue;
            repeatedStoreType = isPrimitive() ? null : RuntimeClasses.getRepeatedStoreType(descriptor.getType());
        ...
        public TypeName getRepeatedStoreType() {
            if (isGroup() || isMessage()) {
                ...
                // primitive types
                return RuntimeClasses.getPrimitiveRepeatedStoreType(descriptor.getType());
            } else if (isString()) {
        ...
        public String getJavadoc() {
            return com.diffbot.primibuf.FieldUtil.getProtoDefinitionLine(descriptor) + "\n";
        }

        public boolean isFixedWidth() {
            return com.diffbot.primibuf.FieldUtil.isFixedWidth(descriptor.getType());
        }

        public int getFixedWidth() {
            checkState(isFixedWidth(), "not a fixed width type");
            return com.diffbot.primibuf.FieldUtil.getFixedWidth(descriptor.getType());
        }
        ...
        public TypeName getPrimitiveType() {
            return RuntimeClasses.getPrimitiveStoreType(descriptor.getType());
        }
        ...
        public TypeName getOptionalReturnType() {
            ...
            if (typeName.equals(ArrayTypeName.of(TypeName.BYTE))) {
                // NOTE: byte[] is not considered as a repeated field
